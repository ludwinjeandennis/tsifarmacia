@extends("layouts.app")

@section('title' , 'Ver')

@section('style')

@endsection

@section("header","Pedidos")

@section("breadcrumb")

    <li class="breadcrumb-item"><a href="{{route("index")}}">Inicio</a></li>
    <li class="breadcrumb-item"><a href="{{route("orders.index")}}">pedidos</a></li>
    <li class="breadcrumb-item">ver pedido</li>

@endsection

@section('content')

<div class="card card-primary">
        <!-- /.card-header -->
        <!-- Inicio del formulario -->

            <div class="card-body">

                <div class="form-group">
                    <label>Nombre del Usuario</label>
                    <input type="text" value="{{$order->user->name}}" class="form-control" disabled>
                </div>

                <div class="form-group">
                    <label>¿Está Asegurado?</label>
                    <input type="text" value="{{$order->is_insured}}" class="form-control" disabled>
                </div>

                <div class="form-group">
                    <label>Farmacia</label>
                    <input type="text" value="{{$order->pharmacy->name ?? ''}}" class="form-control" disabled>
                </div>

                <div class="form-group">
                    <label>Nombre del Doctor</label>
                    <input type="text" value="{{$order->doctor->name ?? 'N/A'}}" class="form-control" disabled>
                </div>

                <div class="form-group">
                    <label>Estado</label>
                    <input type="text" value="{{$order->status}}" class="form-control" disabled>
                </div>
                @if($order->user->hasRole('client'))
                <div class="row d-flex justify-content-center ">
                <div id="carouselExampleControls" class="carousel carousel-dark slide" data-bs-ride="carousel">
                <label>Receta del Cliente</label>
                    <div class="carousel-indicators">
                    @foreach($prescriptions as $prescription)
                        <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="{{$loop->index}}" @if ($loop->first) aria-current="true" class="active" @endif aria-label="Diapositiva {{$loop->iteration}}"></button>
                    @endforeach
                    </div>
                    <div class="carousel-inner ">
                        @foreach($prescriptions as $prescription)
                        <div class="carousel-item @if($loop->first) active @endif">
                            <img src="{{asset($prescription->path)}}" class="cus-img" alt="imagen de receta">
                        </div>
                        @endforeach

                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Anterior</span>
                    </button>
                    <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls" data-bs-slide="next">
                        <span class="carousel-control-next-icon" aria-hidden="true"></span>
                        <span class="visually-hidden">Siguiente</span>
                    </button>
                    </div>
                </div>
                @endif
                @if($order->status == 'Processing')
                <label class="text-danger fs-4 mt-4">Por favor, inserte los medicamentos del cliente</label>
                <form action="{{route('orders.assign' , $order->id)}}" method="POST" enctype="multipart/form-data">

                @csrf

                <div class="form-group">

                  <label for="med[]" class="form-label">Medicamento</label>

                  <select class="js-example-basic-multiple select2 @error('med') is-invalid @enderror" name="med[]" multiple="multiple" style="width: 100%;" >

                    @foreach($medicines as $medicine)

                      <option value="{{$medicine->id}}">{{$medicine->name}}</option>

                    @endforeach

                  </select>

                  @error('med')
                      <span class="invalid-feedback fs-6" role="alert">
                          <strong>{{ $message }}</strong>
                      </span>
                  @enderror
                  </div>

                  <div class="form-group">
                    <label for="qty[]" class="form-label">Cantidad</label>

                    <select class="js-example-basic-multiple select2 @error('qty') is-invalid @enderror" name="qty[]" multiple="multiple" style="width: 100%;">

                    @for($x=1;$x<=10;$x++)
                        <option value="{{$x}}">{{$x}}</option>
                    @endfor
                    </select>

                    @error('qty')
                      <span class="invalid-feedback fs-6" role="alert">
                          <strong>{{ $message }}</strong>
                      </span>
                    @enderror

                  </div>

                <button type="submit" class="btn btn-dark w-100">Asignar</button>

                </form>
                @endif

            </div>
            <!-- /.card-body -->

    </div>

@endsection